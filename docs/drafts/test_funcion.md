...
def get_rate_limit_script():
    ...
    if _rate_limit_script is None:
        _rate_limit_script = r.register_script(LUA_SCRIPT)
...
"""
File: database.py
This module contains the database connection and configuration for the DropVault application. It establishes a connection to the MongoDB database using the URI provided in the environment variables and provides a function to access the database instance.
@copyright: 2026 DropVault Team
@created: 2026-04-16 8:03 PM
@Author: CuSam
...
```
# chú thích: sửa để lưu vào thư mục logs
